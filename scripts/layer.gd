...
func _init(_input_count = 0, _output_count = 0, _activation = null):
	...
	for i in output_count:
		...
		for j in input_count:
			temp_weights.append(randfn(0.0, sqrt(2/input_count)))
...
func apply_gradients(learn_rate: float, batch_size: int):
	...
	for node_out in output_count:
		for node_in in input_count:
			weights[node_out][node_in] -= learn_rate * weight_gradients[node_out][node_in] / batch_size
			
	for node_out in output_count:
		biases[node_out] -= learn_rate * bias_gradients[node_out] / batch_size
...
func _to_string():
	...
	for node_out in output_count:
		for node_in in input_count:
			string += "[" + str(node_in) + " -> " + str(node_out)+ "] " + str(snapped(weights[node_out][node_in], 0.01)) + "\n"
		string += "\n"
